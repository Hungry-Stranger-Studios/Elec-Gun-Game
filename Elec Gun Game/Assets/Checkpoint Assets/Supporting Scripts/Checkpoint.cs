...
public class Checkpoint : MonoBehaviour
{
    ...
    [SerializeField] 
    private Transform respawnSetPos;

    private bool activated = false;

    private void OnTriggerEnter2D(Collider2D other)
    {   
        //Only trigger a checkpoint once
        if (!activated) { 
            activated = true;
            RespawnController con = GetComponentInParent<RespawnController>();
            con.SetRespawnPoint(respawnSetPos.position);
        }
    }
